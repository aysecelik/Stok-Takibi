...
{
    public partial class ÜRÜN_KULLANIMI_DÜZENLE : Form
    {
        public ÜRÜN_KULLANIMI_DÜZENLE()
        {
            InitializeComponent();
        }
        ...
        {
            //ürün adına göre ara
            query = "Select i.ID 'İŞLEM ID',ü.ID 'ÜRÜN ID',ü.ÜrünTürü 'ÜRÜN TÜRÜ',ü.ÜrünKodu 'ÜRÜN KODU',ü.ÜrünAdı 'ÜRÜN ADI',ü.StokKodu 'STOK KODU', ü.StokAdı 'STOK ADI',i.KullanımMiktarı 'KULLANILAN MİKTAR',ü.Birim 'BİRİM',i.Tarih 'İŞLEM TARİHİ',i.[Not] 'NOT' from ÜrünHareketleri i join Ürünler ü on i.Ürün=ü.ID where ü.ÜrünAdı='" + textBox6.Text + "' and ÜrünHareketi='KULLANIM'";
            griddoldur();
        }
        void griddoldur()
        {
        ...
        private void button21_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (checkBox1.Checked == true)
                {
                    ...
                    filtre += " i.Tarih between '" + dateTimePicker1.Value.Date.ToString("dd/MM/yyyy HH:mm:ss") + "' and '" + dateTimePicker2.Value.Date.ToString("dd/MM/yyyy HH:mm:ss") + "'";
        ...
        {
            if (string.IsNullOrEmpty(label16.Text) == true || string.IsNullOrEmpty(textBox11.Text) == true || string.IsNullOrEmpty(textBox10.Text) == true)
            {
                MessageBox.Show("ÜRÜN SEÇİMİ YAPILMALI MİKTAR GİRİLMELİDİR.");
            }
            else if (kalan < Convert.ToDecimal(textBox11.Text + "," + textBox10.Text))
            {
                MessageBox.Show("KALANDAN DAHA FAZLA MİKTAR GİRİLMEMELİDİR.");
            ...
            else
            {
                try
                {
                    ...
                    SqlCommand komutkaydet2 = new SqlCommand("update  ÜrünHareketleri KullanımMiktarı=@p1,Tarih=@p2 set where ID='" + secilen + "'", trans);
                    ...
                    trans.Close();
                    SqlCommand komutkaydet1 = new SqlCommand("update  Ürünler Harcanan=@p1 set where ID='" + ürünid + "'", trans);
                    ...
                    SqlCommand komutkaydet3 = new SqlCommand("update  GünToplam Harcanan=@p1,Gün=@p2 set where Ürün='" + ürünid + "' and Gün='" + date.ToString("yyyyMMdd") + "'", trans);
                    ...
                    komut = new SqlCommand("Select Harcanan,Gün from GünToplam where Gün>'" + dateTimePicker4.Value.ToString("yyyyMMdd") + "' and Ürün='" + ürünid + "'", baglan.baglan());
                    ...
                    komut.CommandText = "Select i.KullanımMiktarı from ÜrünHareketleri i join Ürünler ü on i.Ürün=ü.ID where i.ID='" + dataGridView1.Rows[dataGridView1.CurrentCell.RowIndex].Cells[0].Value.ToString() + "'";
                    object ürünek = komut.ExecuteScalar();
                    decimal ek = Convert.ToDecimal(ürünek);

                    for (int i = 0; i < toplam.Count; i++)
                    {
                        SqlCommand komutkaydet4 = new SqlCommand("update  GünToplam Harcanan=@p1 set where Ürün='" + ürünid + "' and Gün='" + gün[i].ToString("yyyyMMdd") + "'", baglan.baglan());
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            komut.CommandText = "Select ü.Harcanan-i.KullanımMiktarı from ÜrünHareketleri i join Ürünler ü on i.Ürün=ü.ID where i.ID='" + dataGridView1.Rows[dataGridView1.CurrentCell.RowIndex].Cells[0].Value.ToString() + "'";
        ...
        private void ÜRÜN_KULLANIMI_DÜZENLE_Load(object sender, EventArgs e)
        {
